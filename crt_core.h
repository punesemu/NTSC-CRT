...
/* Optional linker symbol prefix when compiling multiple CRT variants into one
 * binary (e.g. -DNTSCCRT_SYM_PREFIX=nesrgb_ on a second object library). Empty
 * by default. */
#ifndef NTSCCRT_SYM_PREFIX
#define NTSCCRT_SYM_PREFIX
#endif
#define NTSCCRT_CONCAT_IMPL(a, b) a##b
#define NTSCCRT_CONCAT(a, b) NTSCCRT_CONCAT_IMPL(a, b)
#define NTSCCRT_SYM(name) NTSCCRT_CONCAT(NTSCCRT_SYM_PREFIX, name)
 ...
 */
extern void NTSCCRT_SYM(crt_init)(struct CRT *v, int w, int h, int f, unsigned char *out);
 ...
 *   out - pointer to output image data
 */
extern void NTSCCRT_SYM(crt_resize)(struct CRT *v, int w, int h, int f, unsigned char *out);

/* Resets the CRT settings back to their defaults */
extern void NTSCCRT_SYM(crt_reset)(struct CRT *v);

/* Modulates RGB image into an analog NTSC signal
 *   s - struct containing settings to apply to this field
 */
extern void NTSCCRT_SYM(crt_modulate)(struct CRT *v, struct NTSC_SETTINGS *s);
    
/* Demodulates the NTSC signal generated by crt_modulate()
 *   noise - the amount of noise added to the signal (0 - inf)
 */
extern void NTSCCRT_SYM(crt_demodulate)(struct CRT *v, int noise);
 ...
 */
extern int NTSCCRT_SYM(crt_bpp4fmt)(int format);
...
extern void NTSCCRT_SYM(crt_sincos14)(int *s, int *c, int n);
